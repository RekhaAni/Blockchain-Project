...
def init_db():
    ...
    CREATE TABLE IF NOT EXISTS blockchain_records (
        id INTEGER PRIMARY KEY AUTOINCREMENT,
        transaction_type TEXT NOT NULL, -- e.g., 'list_for_sale', 'buy_request', 'transfer_approved'
        survey_number TEXT NOT NULL,
        details TEXT NOT NULL, -- JSON string of transaction details
    ...
    cursor.execute("""
        CREATE TABLE IF NOT EXISTS ownership_transfer_requests (
            ...
            created_at TIMESTAMP DEFAULT CURRENT_TIMESTAMP,
            UNIQUE (survey_number, new_owner) -- Prevent duplicate requests for the same property and buyer
        )
    """)
...
def update_officer(officer_id):
    ...
    cursor.execute("""
        SELECT u.id, u.username, o.first_name, o.last_name, o.mobile_number,
               o.government_proof_type, o.government_proof_id, o.address
        FROM users u
    ...
    if request.method == "POST":
        ...
        cursor.execute("""
            ...
            SET first_name = ?, last_name = ?, mobile_number = ?,
                government_proof_type = ?, government_proof_id = ?, address = ?
            WHERE user_id = ?
        """, (first_name, last_name, mobile_number, government_proof_type,
              government_proof_id, address, officer_id))
...
def delete_officer(officer_id):
    ...
    cursor.execute("DELETE FROM officers WHERE user_id = ?", (officer_id,))
...
@app.route("/admin/update-record/<int:record_id>", methods=["GET", "POST"])
def update_record(record_id):
    ...
    if request.method == "POST":
        ...
        cursor.execute("""
            UPDATE land_records
            SET survey_number = ?, kahata = ?, owner_name = ?, father_name = ?,
                    village = ?, address = ?, property_size = ?, size_unit = ?,
                    crop_details = ?, location = ?
                WHERE id = ?
        """, (survey_number, kahata, owner_name, father_name, village, address,
                  property_size, size_unit, crop_details, location, record_id))
        conn.commit()
        conn.close()

        flash("Land record updated successfully.")
        return redirect(url_for("monitor_records"))
    ...
    return render_template("admin/update_record.html", record=record)

@app.route("/admin/delete-record/<int:record_id>", methods=["POST"])
def delete_record(record_id):
    ...
    flash("Land record deleted successfully.")
    return redirect(url_for("monitor_records"))
...
def seller_list_property_for_sale():
    ...
    if request.method == "POST":
        ...
        cursor.execute("""
            SELECT * FROM land_records 
            WHERE survey_number = ? AND owner_name = ?
        """, (survey_number, session["username"]))
        record = cursor.fetchone()
    ...
    # Fetch properties listed for sale by the user
    cursor.execute("""
    ...
    return render_template(
        "seller/list_property_for_sale.html",
        owned_properties=owned_properties,
        selling_lands=selling_lands
    )

# Withdraw Listing Route
@app.route("/seller/withdraw-listing/<string:survey_number>", methods=["POST"])
def withdraw_listing(survey_number):
    if session.get("role") != "public":
        flash("Unauthorized access!")
        return redirect(url_for("home"))
    ...
    cursor.execute("""
        SELECT * FROM sell_alerts 
        WHERE survey_number = ? AND seller = ? AND status = 'active'
    """, (survey_number, session["username"]))
    sell_alert = cursor.fetchone()

    if not sell_alert:
        flash("This property is not listed for sale.")
        conn.close()
        return redirect(url_for("seller_list_property_for_sale"))
    ...
    flash("Property withdrawn from sale successfully.")
    return redirect(url_for("seller_list_property_for_sale"))
...
def buyer_view_selling_lands():
    ...
    # Fetch available lands for sale excluding the user's own properties
    cursor.execute("""
        SELECT lr.*, sa.seller 
        FROM land_records lr
        JOIN sell_alerts sa ON lr.survey_number = sa.survey_number
        WHERE sa.status = 'active' AND lr.owner_name != ?
    """, (session["username"],))
    selling_lands = cursor.fetchall()
...
def buyer_request_buy(survey_number):
    ...
    cursor.execute("""
        SELECT * FROM sell_alerts
        WHERE survey_number = ? AND status = 'active'
    """, (survey_number,))
    sell_alert = cursor.fetchone()
    ...
    try:
        cursor.execute("""
            INSERT INTO ownership_transfer_requests (survey_number, current_owner, new_owner, request_type, status)
            VALUES (?, ?, ?, 'buy', 'pending')
        """, (survey_number, sell_alert["seller"], session["username"]))
        conn.commit()
        flash("Buy request submitted successfully.")
    except sqlite3.IntegrityError:
        flash("You have already submitted a buy request for this property.")
    finally:
        conn.close()
...
@app.route("/seller/view-buy-requests/<string:survey_number>")
def seller_view_buy_requests(survey_number):
    if session.get("role") != "public":
        flash("Unauthorized access!")
        return redirect(url_for("home"))
    ...
    # Fetch all pending buy requests for the property
    cursor.execute("""
        SELECT * FROM ownership_transfer_requests
        WHERE survey_number = ? AND status = 'pending'
    """, (survey_number,))
    buy_requests = cursor.fetchall()

    conn.close()

    return render_template("seller/view_buy_requests.html", buy_requests=buy_requests, survey_number=survey_number)


@app.route("/seller/accept-buy-request/<int:request_id>", methods=["POST"])
def seller_accept_buy_request(request_id):
    if session.get("role") != "public":
        flash("Unauthorized access!")
        return redirect(url_for("home"))
    ...
    cursor.execute("""
        ...
        WHERE id = ?
    """, (request_id,))
    buy_request = cursor.fetchone()

    if not buy_request or buy_request["status"] != "pending":
        flash("Invalid or already processed buy request.")
        conn.close()
    ...
    cursor.execute("""
        ...
        SET owner_name = ?
        WHERE survey_number = ?
    """, (buy_request["new_owner"], buy_request["survey_number"]))

    # Mark the buy request as accepted
    cursor.execute("""
        UPDATE ownership_transfer_requests
        SET status = 'approved'
        WHERE id = ?
    ...
    # Mark other pending requests for the same property as rejected
    cursor.execute("""
        UPDATE ownership_transfer_requests
        SET status = 'rejected'
        WHERE survey_number = ? AND status = 'pending' AND id != ?
    """, (buy_request["survey_number"], request_id))

    # Mark the sell alert as inactive
    cursor.execute("""
        UPDATE sell_alerts
        SET status = 'inactive'
        WHERE survey_number = ?
    """, (buy_request["survey_number"],))

    conn.commit()
    conn.close()

    flash("Buy request accepted successfully.")
    return redirect(url_for("seller_dashboard"))


@app.route("/seller/reject-buy-request/<int:request_id>", methods=["POST"])
def seller_reject_buy_request(request_id):
    if session.get("role") != "public":
        flash("Unauthorized access!")
        return redirect(url_for("home"))
    ...
    # Mark the buy request as rejected
    cursor.execute("""
        UPDATE ownership_transfer_requests
        SET status = 'rejected'
        WHERE id = ?
    ...
    flash("Buy request rejected successfully.")
    return redirect(url_for("seller_dashboard"))

# View Owned Properties Route
...
def seller_view_owned_properties():
    ...
    return render_template(
        "seller/view_owned_properties.html",
        owned_properties=owned_properties,
        selling_lands=selling_lands
    )
...
def admin_dashboard():
    ...
    cursor.execute("SELECT COUNT(*) AS total FROM land_records")
    total_land_records = cursor.fetchone()["total"]

    # Fetch total officers
    cursor.execute("SELECT COUNT(*) AS total FROM users WHERE role = 'officer'")
    total_officers = cursor.fetchone()["total"]

    # Fetch active sell alerts
    cursor.execute("""
        SELECT COUNT(*) AS total FROM sell_alerts 
        WHERE status = 'active'
    """)
    active_sell_alerts = cursor.fetchone()["total"]
    ...
    return render_template(
        ...
        active_sell_alerts=active_sell_alerts
    )
...
def admin_blockchain_records():
    if session.get("role") != "admin":
        flash("Unauthorized access!")
        return redirect(url_for("home"))

    return render_template("admin/blockchain_records.html", blockchain=blockchain.chain)
...
def admin_blockchain_integrity():
    ...
    for i in range(1, len(blockchain)):
        ...
        # Additional validation (e.g., proof-of-work) can be added here
...
def initiate_transfer():
    ...
    try:
        ...
        flash("An ownership transfer request al ready exists for this property.")
